...
module.exports = class MultiBuffer
{
    ...
    indexOf( value, offset, encoding )
    {
        ...
        if( !offset ){ offset = 0; }
        if( value.length === 0 ){ return offset < this._length ? offset : -1; }

        let b = 0, buff = this._buffers[0], i = 0, index = 0, until = this._length - value.length;
        ...
        while( index <= until )
        {
            if( i + value.length <= buff.length )
            {
                let m = buff.indexOf( value, i );
            ...
            {
                let m_buff = buff, m_b = b, m_i = i, matched = 0;

                while( m_buff[m_i] === value[matched] )
                {
                    ...
                    {
                        if( ++m_i >= m_buff.length  )
                        {
                            m_buff = this._buffers[++m_b]; m_i = 0;
                        }
                    }
                }
